...
.. glossary::
  ...
  pulse
    A short burst of signal that is used to control the laser. The pulse is generated by the FPGA and sent to the DAC.

  pulse sequence
    A series of pulses that are used to control the laser. The pulse sequence is generated by the FPGA and sent to the DAC.
  
  waveform
    Also shortened as "wave". A discrete digital representation of the rise and fall of a laser pulse. The waveform is used to control the laser.

  pulse definition
    A set of parameters that define the characteristics of a pulse. Check :meth:`~qlaser_zcu.qlaser_fpga.PulseConfig` for detailed parameters
  
  entry
    A single pulse definition in the pulse definition RAM. Each entry is accumulative in the pulse definition RAM.
  
  start time
    Starting time of the pulse. There need to be at least 5 time units before each pulse starts.

  start address
    The address or location in the waveform table RAM where the base value of the rise of the pulse is stored. This value can be managed by the package itself.

  wave length
    Duration of the rise of the pulse. This value can be managed by the package itself.

  gain factor
    Amplitude (volts) scaling factor of the pulse. This value should always be any decimals between (0, 1].

  time factor
    Step size of the pulse. 10 ns/step. Defines how much/quick of the discrete wavefrom values should be increment/decrement. This value should always be any decimals between [1, wave_len).
